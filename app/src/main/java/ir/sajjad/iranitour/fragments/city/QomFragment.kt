...
import ir.sajjad.iranitour.databinding.FragmentQomBinding
import ir.sajjad.iranitour.databinding.ItemImageShowBinding
import ir.sajjad.iranitour.interfaces.ItemCityEvent

class QomFragment : Fragment(), ItemCityEvent {
    lateinit var binding: FragmentQomBinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentQomBinding.inflate(layoutInflater)
        return binding.root
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val data = arrayListOf<ItemPostCity>(
            ItemPostCity(
                1,
                resources.getString(R.string.jafariyeh),
                "",
                "",
                false
            ),
            ItemPostCity(
                2,
                resources.getString(R.string.qom),
                "",
                "",
                false
            ),
            ItemPostCity(
                3,
                resources.getString(R.string.kahak),
                "",
                "",
                false
            )
        ...
        binding.recyclerQom.layoutManager =
            LinearLayoutManager(context, RecyclerView.VERTICAL, false)
        binding.recyclerQom.setHasFixedSize(true)
        binding.recyclerQom.adapter = adaptersCity
        val animation = AnimationUtils.loadAnimation(context, R.anim.item_enter_anim)
        val controller = LayoutAnimationController(animation)
        binding.recyclerQom.layoutAnimation = controller
